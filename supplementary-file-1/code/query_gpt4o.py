...
"""
    ...
    # Make API request with 30-minute timeout
...
def run_single_query(client: OpenAI, row: pd.Series, output_dir: str, index: int, total: int) -> Dict:
    ...
    try:
        ...
        # Make API request
        response = create_api_request(client, target, indication, mesh)
...
def run_batch_queries(df: pd.DataFrame, output_dir: str, n_workers: int):
    ...
    with ThreadPoolExecutor(max_workers=n_workers) as executor:
        # Submit all tasks
        future_to_row = {
            executor.submit(run_single_query, client, row, output_dir, idx + 1, total): (idx, row)
...
Examples:
  ...
  python query_pooled.py --workers 4 --output results

  # Process entire dataset with 8 workers
  python query_pooled.py -w 8 -o outputs

  # Using custom data file
  python query_pooled.py -w 4 -o results --data custom_data.tsv
        """
    )
