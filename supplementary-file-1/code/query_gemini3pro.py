...
# Gemini 3 Pro with thinking mode enabled. `thinking_level` is the Gemini 3 way of
# controlling reasoning depth (the older `thinking_budget` is ignored by Gemini 3) and
# requires a Gemini 3-era SDK; see ../requirements for the minimum google-genai version.
MODEL_NAME = "gemini-3-pro-preview"

# 30-minute request timeout (milliseconds), matching the GPT-5 runs.
...
def create_api_request(client: genai.Client, target_symbol: str, indication_name: str, mesh: str,
                       thinking_level: str = "high"):
    """Create and send the API request with replaced template variables."""

    # Developer prompt template (passed to Gemini as the system instruction)
...
"""
    ...
    # JSON Schema (identical to the schema used for GPT-5/GPT-4o; passed to Gemini as
    # a standard JSON Schema via `response_json_schema`)
    json_schema = {
        "type": "object",
        "properties": {
            "target": {
    ...
    # Make API request. The developer prompt is passed as the system instruction (the
    # Gemini counterpart of the OpenAI "developer" role), so the rubric stays invariant
    # across T-I pairs and only the user message varies. No tools are attached (no Google
    # Search grounding, no function calling), so generation relies solely on the model's
    # internal knowledge, as for GPT-5.
    response = client.models.generate_content(
        model=MODEL_NAME,
        contents=user_content,
        config=types.GenerateContentConfig(
            system_instruction=developer_content,
...
def extract_output(response) -> Tuple[Optional[str], List[str], Optional[str]]:
    """Split the model output into the JSON answer text, thought summaries, and finish reason.

    With include_thoughts=True the first candidate carries both thought-summary parts
    (part.thought == True) and answer parts; only the latter form the JSON answer.
    """
    ...
    if candidates:
        ...
        finish_reason = str(candidate.finish_reason) if candidate.finish_reason is not None else None
    ...
    return answer_text, thought_parts, finish_reason


def response_to_dict(response) -> dict:
    """Serialize the raw SDK response (thought signatures are bytes, hence JSON mode)."""
    if hasattr(response, "to_json_dict"):
        return response.to_json_dict()
    return response.model_dump(mode="json", exclude_none=True)


def run_single_query(client: genai.Client, row: pd.Series, output_dir: str, index: int, total: int,
                     thinking_level: str = "high") -> Dict:
    """Run a single query using the Gemini API directly."""
    target = row['gene']
    ...
    try:
        ...
        response = create_api_request(client, target, indication, mesh, thinking_level)

        # Separate the JSON answer from thought summaries and parse it
        answer_text, thought_summaries, finish_reason = extract_output(response)
        try:
            parsed = json.loads(answer_text) if answer_text else None
        except json.JSONDecodeError:
            parsed = None
        if parsed is None:
            print(f"   ⚠️  No valid JSON in model output (finish_reason={finish_reason})")
...
def run_batch_queries(df: pd.DataFrame, output_dir: str, n_workers: int,
                      thinking_level: str = "high"):
    """Run queries concurrently using ThreadPoolExecutor with shared Gemini client."""
    total = len(df)
    results = []
    ...
    # Create shared Gemini client (API key read from GOOGLE_API_KEY / GEMINI_API_KEY)
...
Examples:
  ...
  python query_gemini3pro.py -w 5 -o result/gemini3pro --data data/test.tsv

  # Single query mode without MeSH ID (uses indication name as identifier)
  python query_gemini3pro.py --single --target TP53 --indication "Lung adenocarcinoma" -o result/gemini3pro

  # Single query with a lower thinking level
  python query_gemini3pro.py --single --target BRCA1 --indication "Breast cancer" -o result/gemini3pro --thinking-level low
        """
    )
    ...
    # Gemini 3 Pro specific arguments
    parser.add_argument(
        '--thinking-level',
        type=str,
        choices=['low', 'high'],
